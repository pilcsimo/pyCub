...
## Task
Implement function move() in `smooth_movements.py` that will move the robot's arm in a straight line or a circle.  
...
## Scoring
 - you will be given N random test cases
   - each will be awarded with 0.5 points -> max N/2 points
 ...
 - the code should be as general as possible to work for any allowed input. Meaning that it should not be, for example, 
   a long if/else (switch) statement for each axis etc. On top of that, the quality of code will be also checked. 
   Maximum of P points will be awarded for this criterion
